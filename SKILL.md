---
name: xiaohongshu-notes-search
description: 小红书高赞笔记搜索工具。支持关键词获取小红书实时高赞（高收藏、高评论）笔记数据，助力创作者发现热门趋势、获取创作灵感。支持四大能力：(1) 关键词搜索笔记/视频，可按点赞数、评论数、收藏数、发布时间、内容类型筛选排序；(2) 博主作品抓取，按主页链接获取博主的互动数据（粉丝量、点赞量或收藏量等）或公开作品列表；(3) 笔记（视频）详情，获取详情数据及互动数据等，分析笔记的市场表现；(4) 笔记评论分析，按笔记链接获取评论内容与互动数据。用户提到小红书/xhs/rednote 且需要查数据、市场调研、舆情监测、做选题、竞品监控、KOL筛选、舆情分析时调用；无需登录账号
license: MIT
version: 1.1.4
category: 数据分析
...
> ✨ **一句话**：小红书高赞笔记搜索工具。支持关键词获取小红书实时高赞（高收藏、高评论）笔记数据，助力创作者发现热门趋势、获取创作灵感。支持关键词搜索排序、博主作品抓取、笔记评论分析；用于小红书或RedNote数据分析、市场调研、达人筛选、选品、找品、爆品、趋势、市场容量、竞争结构等数据形成经营判断时使用，依据小红书真实返回的数据形成分析结论。
...
> **Note:** 请先通过 [小红书高赞笔记检索技能](https://www.guaikei.com) 开通TOKEN，配置环境变量 `GUAIKEI_API_TOKEN` 后才能正常运行。
...
# 获取一年内评论，剔除一年前无效评论
node src/xiaohongshu/comment-cli.js --url "https://www.xiaohongshu.com/explore/xxx?xsec_token=yyy" --limit 1000 --expire 365
...
- 官网 / TOKEN 开通：[小红书笔记检索SKILL](https://www.guaikei.com)
